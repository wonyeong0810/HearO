...
# 모든 모델이 import 되어야 metadata 가 채워진다.
...
def _include_object(obj, name, type_, reflected, compare_to) -> bool:  # type: ignore[no-untyped-def]
    # 확장(pg_trgm)이 만든 객체는 autogenerate 대상에서 제외
    return not (type_ == "table" and name in {"pg_stat_statements"})
